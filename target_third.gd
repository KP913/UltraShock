...
func _ready():
	...
	$note2.position = Vector2(0,main.NOTE_DIRECTION*128*duration*main.speed_mult).rotated(angle)
	
	
	#if OS.get_name() != "Android": flick = 0
	
	#var tween = get_tree().create_tween()
	#tween.tween_property($note,"position",-$note.position,b2s(4*2))
	#tween.finished.connect(_on_Tween_tween_all_completed)
	#note_tween = tween
	#if hold:
		#var tween2 = get_tree().create_tween()
		#tween2.tween_property($note2,"position",-$note.position,b2s(4*2+duration))
		#tween2.bind_node($note2)
		#hold_tween = tween2
	
	$Sprite2D.frame_coords.x = 2*flick+int(double)
	$Sprite2D.frame_coords.y = button
	$note/Sprite2D.frame_coords.x = 2*flick+int(double)
	$note/Sprite2D.frame_coords.y = button
	$note2/Sprite2D.frame_coords.x = 2*flick+int(double)
	$note2/Sprite2D.frame_coords.y = button
...
#		tween.stop()
	...
	if chance:
		$note/Sprite2D.material = ShaderMaterial.new()
		$note/Sprite2D.material.shader = main.rainbow_shader
...
func _process(delta):
	...
	#print("aaa      ",position,"   ;   ",global_position,"   ;   ",$note.position,"   ;   ",$note.global_position)
	#print($note.global_position)
	#print($note.position.y)
	if stopped != null: $note.global_position = stopped
	timing = b2s(start_time+4/main.speed_mult) - (main.beat - start_beat)*60/main.bpm
	if timing == 0: hit()
	#if time == 512: debug(str(timing))
	position += Vector2(128*main.speed_mult*s2b(delta),0).rotated(PI/2+angle)
	#position = Vector2(main.LEFT_SIDE,0) + (position.rotated(-PI/2-angle) + Vector2(128*main.speed_mult*s2b(delta),0)).rotated(PI/2+angle)
		...
		if (double && ((held1 && press2) or (press1 && held2))) or (!double && (press1 or press2)) && !((second_voice && main.frame_hit2) or (!second_voice && main.frame_hit1)):
			...
			if hold:
				...
				if q != "womp womp":
					...
					#if note_tween != null:
						#note_tween.kill()
					if OS.get_name() == "Android": flick_hold = flick
			elif flick:
