...
const RECEIVER_Y = 384
const LEFT_SIDE = 448
const RIGHT_SIDE = 512
...
func spawn_note(info):
	...
	var note = preload("res://target_second.tscn").instantiate()
	
	if info.second_voice: note.position.x = RIGHT_SIDE + snapped(randf_range(0,200),16)
	else: note.position.x = LEFT_SIDE - snapped(randf_range(0,200),16)
	note.position.y = RECEIVER_Y-512 - $notes.global_position.y + (beat-((info.time/64)-4/speed_mult))*128*speed_mult #160+512
